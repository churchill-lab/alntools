...
class Sparse3DMatrix(object):
    ...
    def _reconstruct_spmat(self, h5fh, hid, datanode, dtype):
        try:
            mtype = h5fh.get_node_attr(datanode, "mtype").decode("utf-8")
            incidence_only = h5fh.get_node_attr(datanode, "incidence_only")
        except AttributeError:
            mtype = "coo_matrix"
            incidence_only = False
        hapnode = h5fh.get_node(f"{datanode}/h{hid}")
        if mtype == "csc_matrix":
            ...
            if multiplier.ndim == 1:
                ...
                if (
                    ...
                    for hid in range(self.shape[1]):
                        ...
                        self.data[hid].data *= multiplier_vec.repeat(
                            np.diff(self.data[hid].indptr)
                        )
                else:
                    raise RuntimeError("The axis should be 0, 1, or 2.")
            elif isinstance(
                multiplier, Sparse3DMatrix
            ):  # multiplier is Sparse3DMatrix object
                for hid in range(self.shape[1]):
                    self.data[hid] = self.data[hid].multiply(multiplier.data[hid])
            else:
                raise RuntimeError(
                    "The multiplier should be 1, 2 dimensional numpy array or a Sparse3DMatrix object."
                )
        else:
            raise RuntimeError("The original matrix must be finalized.")
    ...
    def combine(self, other):
        if self.finalized and other.finalized:
            ...
            for hid in range(self.shape[1]):
                dmat.data.append(
                    vstack([self.data[hid], other.data[hid]])
                )  # WARNING: vstack returns coo_matrix!!
            dmat.finalize()
            return dmat
        else:
            raise RuntimeError("Both matrices must be finalized.")
        ...
        if self.finalized:
            ...
            for hid in range(self.shape[1]):
                ...
                i1 = h5fh.create_carray(
                    hgroup, "indptr", obj=spmat.indptr.astype(index_dtype), filters=fil
                )
                ...
                if not incidence_only:
                    d = h5fh.create_carray(
                        hgroup, "data", obj=spmat.data.astype(data_dtype), filters=fil
                    )
            h5fh.flush()
            h5fh.close()
        else:
