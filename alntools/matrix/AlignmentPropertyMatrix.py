...
class AlignmentPropertyMatrix(Sparse3DMatrix):
    ...
    def __init__(
        ...
        if (
            ...
            with open(ecfile, "rb") as f:
                ...
                if ecformat == 2:
                    ...
                    for hidx in range(self.num_haplotypes):
                        ...
                        hname.append(
                            unpack("<{}s".format(hname_len), f.read(hname_len))[
                                0
                            ].decode("utf-8")
                        )
                    ...
                    for tidx in range(self.num_loci):
                        ...
                        tname.append(
                            unpack("<{}s".format(tname_len), f.read(tname_len))[
                                0
                            ].decode("utf-8")
                        )
                        for hidx in range(self.num_haplotypes):
                    ...
                    for sidx in range(self.num_samples):
                        ...
                        sname.append(
                            unpack("<{}s".format(sname_len), f.read(sname_len))[
                                0
                            ].decode("utf-8")
                        )
                    ...
                    indptr_A = np.array(
                        unpack("<{}i".format(indptr_len), f.read(4 * indptr_len))
                    )
            ...
            if not shallow:
                ...
                if h5fh.__contains__(f"{metanode}/sname"):
            ...
            if h5fh.__contains__(f"{datanode}/count"):
                try:
                    ...
                    nmat_node = h5fh.get_node(f"{datanode}/count")
            ...
            if h5fh.__contains__(f"{datanode}/lengths"):
    ...
    def pull_alignments_from(self, reads_to_use, shallow=False):
        ...
        if new_alnmat.count is not None:
            if type(new_alnmat.count) == csc_matrix:
                ...
                raise RuntimeError(
                    "APM count should be either scipy.sparse.csc_matrix or numpy.ndarray"
                )
        return new_alnmat
    ...
    def apply_genotypes(self, gt_file):
        ...
        for h in range(self.num_haplotypes):
        ...
        if self.count is not None:
            if len(self.count.shape) == 1:  # count is a vector
                ...
                cgroup = h5fh.create_group(
                    h5fh.root, "count", "Sparse matrix components for N matrix"
                )
                h5fh.create_carray(
                    cgroup,
                    "indptr",
                ...
                h5fh.create_carray(
                    cgroup, "data", obj=self.count.data.astype(index_dtype), filters=fil
                )
        if not shallow:
            h5fh.set_node_attr(h5fh.root, "hname", self.hname)
            h5fh.create_carray(
                h5fh.root, "lname", obj=self.lname, title="Locus Names", filters=fil
            )
