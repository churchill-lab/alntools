...
def lookup_gene_id(db_con, gene_id):
    c = db_con.cursor()
    c.execute(
        """SELECT gene_id FROM gene_info WHERE UPPER(symbol) = UPPER(?)""", (gene_id,)
    )
...
def add_gene_count_total(cursor, sample_id, gene_id, read_count):
    cursor.execute(
        """INSERT INTO gene_count_totals VALUES (?, ?, ?)""",
        (sample_id, gene_id, read_count),
    )


def add_gene_edge(cursor, sample_id, gene_id_1, gene_id_2, read_count):
    cursor.execute(
        """INSERT INTO gene_edge VALUES (?, ?, ?, ?)""",
        (sample_id, gene_id_1, gene_id_2, read_count),
    )


def add_gene_info(
    cursor, gene_id, chromosome, start_pos_bp, end_pos_bp, strand, symbol, name
):
    cursor.execute(
