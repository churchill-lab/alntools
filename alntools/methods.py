...
from alntools import salmon_utils
...
def salmon2ec(salmon_dir, ec_filename, sample=None, target_filename=None):
    salmon_utils.convert(
        salmon_dir, ec_filename, sample=sample, target_filename=target_filename
    )
...
def generate_bam_ranges(
    ...
    bam_utils.generate_bam_ranges(
        input_files, range_filename, target_filename, temp_dir
    )
...
def emase2db_config(sample_file, directory=None):
    ...
    LOG.info(f"Generating sample file: {sample_file}")
    LOG.info(f"Top level directory: {working_directory}")
            ...
            for file in files:
                if file.endswith(".h5"):
                    ...
                    if sample_name in samples:
                        ...
                        sample_name = f"{sample_name}_{samples[sample_name]}"
                    else:
                        samples[sample_name] = 1
    ...
    LOG.info(
        "Config file created in {}".format(utils.format_time(start_time, time.time()))
    )
...
def emase2db(sample_file, gene_file, db_file):
    ...
    LOG.info(f"Using sample file: {sample_file}")
    LOG.info(f"Using gene information file: {gene_file}")
    LOG.info(f"Generating db file: {db_file}")

    try:
        db_con = db_utils.init_db(db_file)
        chr_regex = re.compile(
            """"(CHR(OMOSOME)?)?([1-9][0-9]*|X|Y|MT?)""", re.IGNORECASE
        )
        ...
        with tempfile.NamedTemporaryFile() as group_file_fd:
            ...
            with open(gene_file, "rb") as gene_fd:
                ...
                for row in gene_info_tab_reader:
                    ...
                    if chr_match:
                        ...
                        group_file_fd.write(
                            "{}\t{}\n".format(
                                gene_id, "\t".join(transcripts.split(","))
                            )
                        )

            LOG.info(
                "Genes inserted in {}, total time: {}".format(
            ...
            with open(sample_file) as sample_fd:
                ...
                for row in sample_reader:
                    ...
                    LOG.info(f"Parsing: {emase_file_name}")
                    ...
                    for nonzero_index_pair in np.transpose(cntmat.nonzero()):
                        ...
                        if i1 == i2:
                            db_utils.add_gene_count_total(
                                cursor, sample_id, alnmat.lname[i1], read_count
                            )
                        else:
                            db_utils.add_gene_edge(
                                cursor,
                                sample_id,
                                alnmat.lname[i1],
                                alnmat.lname[i2],
                    ...
                    LOG.info(
                        "Parsed in: {}, total time: {}".format(
                            utils.format_time(apm_temp_time, time.time()),
                            utils.format_time(start_time, time.time()),
                        )
                    )

            LOG.info(
                "All counts and edges inserted in: {}, total time: {}".format(
                    utils.format_time(temp_time, time.time()),
                    utils.format_time(start_time, time.time()),
                )
            )
    ...
    LOG.info(
        "Database created in {}".format(utils.format_time(start_time, time.time()))
    )


def parsefastqtest(input_file, chunks, dir):
    start_time = time.time()
    LOG.info(f"Using fastq file: {input_file}")
    barcode_utils.parse(input_file, chunks, dir)
    LOG.info(
        "FAstq file parsed in {}".format(utils.format_time(start_time, time.time()))
    )
